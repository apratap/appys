...
def temp_map_reads_with_BWA(bwa_obj):                
    ...
    p2 = subprocess.Popen(sam_to_bam,stdin = p1.stdout,stdout=open(bwa_obj.mappedBam,'w'),stderr=bwa_map_log_fh)
    p2.communicate()
    p1.stdout.close()
...
def creat_bwa_ref_index(reference,logger,outPath=os.getcwd()):
    ...
    expected_bwa_index_file_suffixes = ['amb','ann','bwt','pac','rbwt','rpac','rsa','sa', 'fai']
